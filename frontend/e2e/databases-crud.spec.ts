...
test.describe("Databases CRUD Operations", () => {
  ...
  test("should create, edit and archive database with confirmations", async ({
    page,
  }) => {
    ...
    await expect(
      page.getByRole("heading", { name: "Register New Database" }),
    ).toBeVisible();
    ...
    await page.getByLabel("Host Name").fill("e2e-db-host.local");
    ...
    await editBtn.click();
    await expect(
      page.getByRole("heading", { name: "Update Database Details" }),
    ).toBeVisible();
...
test.describe("Databases Role-Based Restrictions", () => {
  ...
  test("should restrict viewer from adding databases", async ({ page }) => {
    await page.goto("/dashboard/databases");
    await expect(
      page.getByRole("heading", { name: "Relational Database Inventory" }),
    ).toBeVisible({ timeout: 10000 });
    await expect(
      page.getByRole("button", { name: "Add Database" }),
    ).not.toBeVisible();
