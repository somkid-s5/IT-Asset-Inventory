...
export interface KBDocument {
  ...
  vmLinks?: Array<{
    vm: { id: string; systemName: string; primaryIp?: string | null };
  }>;
  databaseLinks?: Array<{
    database: { id: string; name: string; engine?: string | null };
  }>;
}
...
export const kbService = {
  ...
  getDocuments: async (categoryId?: string) => {
    const res = await api.get<KBDocument[]>(
      `/knowledge-base/documents${categoryId ? `?categoryId=${categoryId}` : ""}`,
    );
    return res.data;
  },
  ...
  searchDocuments: async (q: string, limit: number = 50) => {
    const res = await api.get<KBDocument[]>(
  ...
  getPublicDocument: async (id: string) => {
    const res = await api.get<PublicKBDocument>(
      `/knowledge-base/public/documents/${id}`,
    );
    return res.data;
  },
  ...
  updateDocument: async (
    ...
    const res = await api.patch<KBDocument>(
  ...
  uploadImage: async (file: File) => {
    ...
    const path =
      baseUrl.endsWith("/api") && relativeUrl.startsWith("/api/")
