...
export default function DocsPage() {
  ...
  const { data: searchArticles = [], isLoading: searchLoading } = useQuery({
    queryKey: ["kb-document-search", normalizedSearch],
    queryFn: () => kbService.searchDocuments(search.trim(), 50),
  ...
  const filteredRecentArticles = normalizedSearch
    ? searchArticles
    : recentArticles;
  const articlesLoading = normalizedSearch ? searchLoading : recentLoading;
  ...
  const totalDocs = categories.reduce(
    (sum, category) => sum + (category._count?.documents || 0),
    0,
  );
  const updatedThisWeek = recentArticles.filter((article) => {
    const diffDays = Math.ceil(
      Math.abs(Date.now() - new Date(article.updatedAt).getTime()) /
        (1000 * 60 * 60 * 24),
    );
    return diffDays <= 7;
  }).length;
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Technical documentation, procedures, and operational references
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <SummaryTile
          ...
          value={catsLoading ? "--" : totalDocs}
          hint="Across all categories"
          icon={FileText}
        />
        <SummaryTile
          label="Updated This Week"
          value={catsLoading ? "--" : updatedThisWeek}
        ...
        <SummaryTile
          ...
          hint="Documentation areas"
          icon={Layers3}
          iconClassName="text-info"
        />
        <SummaryTile
          label="Recent Views"
          value={recentLoading ? "--" : recentViews}
          hint="Latest 10 documents"
          icon={Eye}
        ...
        <div className="flex flex-col gap-3 border-b border-border/70 bg-muted/30 p-3 sm:p-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              Browse documentation by operational area
            </p>
          </div>
          ...
          {catsLoading ? (
            ...
            <div className="rounded-xl border-2 border-dashed border-border/40 bg-muted/5 px-4 py-12 text-center">
              ...
              <p className="mb-5 mt-1 text-sm text-muted-foreground">
                Start by initializing the library or creating a category.
              </p>
              {user?.role === "ADMIN" && (
                <Button
                  variant="outline"
                    ...
                    <Card className="flex h-full flex-col justify-between rounded-xl border border-border/60 bg-card p-4 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
                      <div>
                        ...
                        <h3
                          ...
                          onClick={() =>
                            router.push(
                              `/dashboard/docs/categories/${category.id}`,
                            )
                          }
                        >
                          {category.name}
      ...
      <Card className="gap-0 overflow-hidden rounded-2xl border border-border/80 bg-card p-0 shadow-md">
        <div className="flex items-center justify-between border-b border-border/70 bg-muted/30 p-3 sm:p-4">
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              {normalizedSearch
                ? `Matching documents across the full library for “${search.trim()}”`
                : "Latest updates across your knowledge base"}
            </p>
          </div>
          <span className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
            {filteredRecentArticles.length} shown
          </span>
        </div>
        ...
        {articlesLoading ? (
          ...
          <div className="m-3 rounded-xl border border-dashed border-border/60 bg-muted/5 px-6 py-10 text-center sm:m-4">
            ...
            <p className="text-sm font-semibold text-muted-foreground">
              {normalizedSearch
                ? `No documents found for "${search.trim()}"`
          ...
          <div className="grid grid-cols-1 gap-3 p-3 md:grid-cols-2 xl:grid-cols-3 sm:p-4">
            {(normalizedSearch
              ...
              : filteredRecentArticles.slice(0, 6)
            ).map((article) => (
              <Link
                key={article.id}
              ...
              >
                <Card className="h-full rounded-xl border border-border/60 bg-card p-4 transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-lg">
                  <div className="flex items-start justify-between gap-3">
                    <Badge
                      variant="secondary"
                      className="max-w-[75%] truncate text-[10px] uppercase tracking-tight"
                    >
