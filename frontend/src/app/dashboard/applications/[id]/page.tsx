...
export default function ApplicationDetailPage() {
  ...
  const updateAccess = useMutation({ mutationFn: () => api.patch(`/applications/${params.id}/access/${editingAccess?.id}`, { label: editingAccess?.label, address: editingAccess?.address, method: editingAccess?.method, environmentId: editingAccess?.environmentId || null }), onSuccess: () => { invalidate(); setEditingAccess(null); toast.success("Access point updated"); }, onError: () => toast.error("Could not update access point") });
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={Boolean(editingAccess)} onOpenChange={(open) => !open && setEditingAccess(null)}><DialogContent><DialogHeader><DialogTitle>Edit access point</DialogTitle></DialogHeader><div className="grid gap-3"><Input aria-label="Edit access label" placeholder="Label" value={editingAccess?.label ?? ""} onChange={(event) => setEditingAccess((current) => current ? { ...current, label: event.target.value } : current)} /><Input aria-label="Edit access address" placeholder="Address / URL" value={editingAccess?.address ?? ""} onChange={(event) => setEditingAccess((current) => current ? { ...current, address: event.target.value } : current)} /><Input aria-label="Edit access method" placeholder="Method (HTTPS, SSH)" value={editingAccess?.method ?? ""} onChange={(event) => setEditingAccess((current) => current ? { ...current, method: event.target.value } : current)} /><Select value={editingAccess?.environmentId || "none"} onValueChange={(value) => setEditingAccess((current) => current ? { ...current, environmentId: value === "none" ? "" : value } : current)}><SelectTrigger><SelectValue placeholder="Environment (optional)" /></SelectTrigger><SelectContent><SelectItem value="none">Application-wide</SelectItem>{data.environments.map((env) => <SelectItem key={env.id} value={env.id}>{env.name}</SelectItem>)}</SelectContent></Select><Button className="w-full" onClick={() => updateAccess.mutate()} disabled={!editingAccess?.label.trim() || !editingAccess?.address.trim() || !editingAccess?.method.trim() || updateAccess.isPending}>Save access point</Button></div></DialogContent></Dialog>
      ...
      <Dialog open={Boolean(componentDialog)} onOpenChange={(open) => !open && setComponentDialog(null)}><DialogContent><DialogHeader><DialogTitle>Add component</DialogTitle></DialogHeader><div className="space-y-4"><Label htmlFor="component-name">Component name</Label><Input id="component-name" value={componentName} onChange={(event) => setComponentName(event.target.value)} placeholder="e.g. API" /><Label>Assets</Label><select aria-label="Component assets" multiple value={componentLinks.assetIds} onChange={(event) => setComponentLinks({ ...componentLinks, assetIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.assets.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Label>Virtual machines</Label><select aria-label="Component virtual machines" multiple value={componentLinks.vmIds} onChange={(event) => setComponentLinks({ ...componentLinks, vmIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.vms.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Label>Logical databases</Label><select aria-label="Component logical databases" multiple value={componentLinks.logicalDatabaseIds} onChange={(event) => setComponentLinks({ ...componentLinks, logicalDatabaseIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.logicalDatabases.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Button className="w-full" onClick={() => addComponent.mutate()} disabled={!componentName.trim() || addComponent.isPending}>Add component</Button></div></DialogContent></Dialog>
      <Dialog open={Boolean(editingComponent)} onOpenChange={(open) => !open && setEditingComponent(null)}><DialogContent><DialogHeader><DialogTitle>Edit component</DialogTitle></DialogHeader><div className="space-y-4"><Label htmlFor="edit-component-name">Component name</Label><Input id="edit-component-name" value={editingComponent?.name ?? ""} onChange={(event) => setEditingComponent((current) => current ? { ...current, name: event.target.value } : current)} /><Label>Assets</Label><select aria-label="Edit component assets" multiple value={componentLinks.assetIds} onChange={(event) => setComponentLinks({ ...componentLinks, assetIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.assets.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Label>Virtual machines</Label><select aria-label="Edit component virtual machines" multiple value={componentLinks.vmIds} onChange={(event) => setComponentLinks({ ...componentLinks, vmIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.vms.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Label>Logical databases</Label><select aria-label="Edit component logical databases" multiple value={componentLinks.logicalDatabaseIds} onChange={(event) => setComponentLinks({ ...componentLinks, logicalDatabaseIds: Array.from(event.target.selectedOptions, (option) => option.value) })} className="min-h-16 w-full rounded-xl border border-border bg-background px-2 py-1 text-sm">{topologyOptions.logicalDatabases.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select><Button className="w-full" onClick={() => updateComponent.mutate()} disabled={!editingComponent?.name.trim() || updateComponent.isPending}>Save component</Button></div></DialogContent></Dialog>
