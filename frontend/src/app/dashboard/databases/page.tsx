...
import {
  ...
  type DatabaseInventoryListResponse,
...
export default function DbPage() {
  ...
  const initialEnvironment = searchParams.get(
    "environment",
  ) as DatabaseEnvironment | null;
  const [showArchived, setShowArchived] = useState(
    () => searchParams.get("archived") === "true",
  );
  const [activeEnvironment, setActiveEnvironment] = useState<
    "ALL" | DatabaseEnvironment
  >(
    initialEnvironment && ["PROD", "UAT", "TEST"].includes(initialEnvironment)
      ? initialEnvironment
      : "ALL",
  );
  ...
  const [pagination, setPagination] = useState({
    ...
    pageSize: Math.min(
      Math.max(Number(searchParams.get("limit") ?? "20"), 10),
      200,
    ),
  });
  ...
  const {
    ...
    queryFn: async () => {
      const response = await api.get<DatabaseInventoryListResponse>(
        "/databases",
        {
          params: {
            ...
            environment:
              activeEnvironment === "ALL" ? undefined : activeEnvironment,
  ...
  const databases = databasePage?.data ?? [];
  const countsByEnvironment = databasePage?.environmentCounts ?? {
    ALL: 0,
    PROD: 0,
    UAT: 0,
    TEST: 0,
  ...
  const [databaseToEdit, setDatabaseToEdit] =
    useState<DatabaseInventoryDetail | null>(null);
  const [loadingEditId, setLoadingEditId] = useState<string | null>(null);
  const [deleteTarget, setDeleteTarget] =
    useState<DatabaseInventoryItem | null>(null);
  ...
  }, [searchTerm, activeEnvironment, showArchived, sorting]);

  const hasActiveDatabaseFilter =
    Boolean(searchTerm.trim()) || activeEnvironment !== "ALL" || showArchived;
  ...
  const columns = useMemo<ColumnDef<DatabaseInventoryItem>[]>(
    () => [
      {
        accessorKey: "name",
        header: ({ column }) => (
          <SortableHeader column={column} title="Database Name" />
        ),
        cell: ({ row }) => (
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <Link
                ...
                className="block truncate font-semibold text-foreground underline-offset-4 hover:text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
                onClick={(event) => event.stopPropagation()}
              >
                {row.original.name}
              </Link>
              {row.original.needsContext ? (
                <span
                  className="mt-1 inline-flex rounded-md border border-warning/30 bg-warning/10 px-1.5 py-0.5 text-[9px] font-semibold uppercase tracking-wider text-warning"
                  title={(row.original.contextIssues ?? []).join(", ")}
                >
                  Needs Context
                </span>
              ) : null}
      ...
      },
      {
        ...
        header: "Accounts",
        cell: ({ getValue }) => (
          <span className="text-[12px] font-semibold pl-2 tabular-nums">
            {getValue() as number}
          </span>
        ),
      },
    ...
    [getHostLabel, loadingEditId, router, user, loading, mounted],
  );
  ...
  const table = useReactTable({
    ...
    pageCount: databasePage?.totalPages ?? 0,
    ...
    try {
      ...
      toast.error(
        error?.response?.data?.message ||
          `Failed to ${deleteTarget.status === "ARCHIVED" ? "restore" : "archive"} database`,
      );
    } finally {
  ...
  const handleExport = () => {
    const exportData = databases.map((db) => ({
      Name: db.name,
      Engine: db.engine,
      Version: db.version || "",
      Environment: db.environment || "",
      Host: getHostLabel(db),
      IP: db.ipAddress || "",
      Port: db.port || "",
      Status: db.status || "",
    ...
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground text-pretty">
            Monitor and manage all database instances
          </p>
        </div>
      ...
      <Card className="gap-0 overflow-hidden rounded-2xl border border-border/80 bg-card p-0 shadow-md">
          ...
          <div className="no-scrollbar flex max-w-full items-center gap-1 overflow-x-auto rounded-xl bg-muted/50 p-1">
            {ENVIRONMENT_TABS.map((tab) => (
              ...
              >
                ...
                <Badge
                  variant="neutral"
                  className="ml-1 h-4 px-1 font-mono text-[9px] bg-muted/50"
                >
              ...
              {table.getHeaderGroups().map((headerGroup) => (
                ...
                >
                  {headerGroup.headers.map((header) => (
                    <TableHead
                      key={header.id}
                      className="border-b border-border px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground"
                    >
                      {flexRender(
              ...
              {isLoading ? (
                ...
                table.getRowModel().rows.map((row) => (
                  <TableRow
                    ...
                    onClick={() =>
                      router.push(`/dashboard/databases/${row.original.id}`)
                    }
                  >
        ...
        <div className="flex flex-col gap-3 border-t border-border/50 bg-muted/10 p-3 sm:flex-row sm:items-center sm:justify-between sm:p-4">
          <div className="text-xs text-muted-foreground">
            Total {databasePage?.total ?? 0} items
          </div>
      ...
      {dialogOpen && (
        <DatabaseFormDialog
          ...
          onOpenChange={(open) => {
            setDialogOpen(open);
            if (!open) setDatabaseToEdit(null);
          }}
      ...
      >
        <DialogContent className="sm:max-w-[425px] rounded-2xl">
          ...
          <div className="py-4 text-sm text-muted-foreground">
            ...
            {deleteTarget?.status === "ARCHIVED" ? "restore" : "archive"}{" "}
            database{" "}
            <span className="font-bold text-foreground">
              {deleteTarget?.name}
            </span>{" "}
            and keep all associated accounts?
          </div>
          <div className="flex justify-end gap-3">
            ...
            <Button
              variant={
                deleteTarget?.status === "ARCHIVED" ? "default" : "destructive"
              }
  ...
  return (
    ...
    >
      ...
      {column.getIsSorted() === "asc" ? (
        ...
        <ChevronsUpDown className="ml-1 h-3 w-3 opacity-0 group-hover:opacity-100" />
      )}
    </button>
  );
}
