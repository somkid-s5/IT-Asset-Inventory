...
export default function DatabaseDetailPage() {
  ...
  const getRoleBadge = (role?: string | null) => {
    ...
    if (normalizedRole.includes("dba")) return "border-warning/30 bg-warning/10 text-warning";
    if (normalizedRole.includes("report")) return "border-success/30 bg-success/10 text-success";
    if (normalizedRole.includes("app")) return "border-low/30 bg-low/10 text-low";
    if (normalizedRole.includes("dev")) return "border-info/30 bg-info/10 text-info";
    return "border-border bg-muted text-foreground";
  };

  const getPrivilegeBadge = (privilege: string) => {
    const p = privilege.toLowerCase();
    if (p.includes("delete") || p.includes("all")) return "border-critical/20 bg-critical/10 text-critical";
    if (p.includes("insert") || p.includes("update") || p.includes("write")) return "border-warning/20 bg-warning/10 text-warning";
