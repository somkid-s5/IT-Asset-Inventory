...
export default function VmDetailPage() {
  ...
  const handleCopyPassword = async (
    account: VmInventoryDetail["guestAccounts"][number],
  ) => {
  ...
  const getLifecycleBadge = (state: VmInventoryDetail["lifecycleState"]) => {
    if (state === "ACTIVE")
      return {
        label: "Active",
        class: "border-success/25 bg-success/10 text-success",
      };
    if (state === "DRAFT")
      return { label: "Draft", class: "border-low/25 bg-low/10 text-low" };
    return {
      label: "Archived",
  ...
  const getSyncBadge = (state: VmInventoryDetail["syncState"]) => {
    ...
    if (normalizedState === "ready to sync")
      return {
        label: "Ready to Sync",
        class: "border-info/25 bg-info/10 text-info",
      };
    if (normalizedState === "connection failed")
      return {
    ...
    return {
      label: "Missing",
      class: "border-warning/25 bg-warning/10 text-warning",
    };
  };
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1 break-words text-xs opacity-90">
                {vm.lifecycleState === "DRAFT"
                  ? "Draft record created from vCenter discovery. Review the context before promoting it."
                  : vm.lifecycleState === "DELETED_IN_VCENTER"
                    ? "This VM is no longer present in vCenter. Archive or delete the record after review."
                    : "This VM is active and synced with the connected source."}
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="relative flex flex-col gap-4 p-4 sm:p-6 lg:flex-row lg:items-start lg:justify-between">
              ...
              <div className="relative z-10 flex min-w-0 items-start gap-4">
                ...
                <div className="space-y-2 min-w-0 flex-1">
                  <div className="flex min-w-0 max-w-full flex-wrap items-center gap-2 sm:gap-3">
                    <h1
                      className="max-w-full truncate text-2xl font-bold tracking-tight text-foreground sm:max-w-[400px]"
                      title={vm.name}
                    >
                      {vm.name}
                    </h1>
                    <span
                      className={cn(
                        "inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider shrink-0",
                    ...
                    <span
                      className={cn(
                        "inline-flex items-center rounded-full border px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider border-border bg-muted/50 text-foreground shrink-0",
                      )}
                    >
                      {vm.powerState}
                    </span>
                  ...
                  <div className="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-2 pt-1 text-sm text-muted-foreground">
                    <span className="min-w-0 max-w-full truncate font-semibold text-foreground/80 sm:max-w-[250px]">
                      {vm.systemName}
                    </span>
              ...
              <div className="relative z-10 flex shrink-0 flex-wrap items-center gap-2 lg:justify-end">
                <div className="flex min-w-[84px] flex-col items-center justify-center rounded-lg border border-border/50 bg-background/50 px-3 py-1.5 shadow-sm">
                  <span className="mb-0.5 flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              ...
              >
                  ...
                  <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                        <Server className="h-4 w-4" /> Connection Information
                      </h3>
                      <div className="space-y-3">
                        <DetailRow
                          label="vCenter Source"
                          value={vm.vcenterName}
                        />
                        <DetailRow
                          label="MoID"
                          value={
                            <span className="font-mono text-xs">{vm.moid}</span>
                          }
                        />
                        <DetailRow
                          label="IP Address"
                    ...
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                        <Clock className="h-4 w-4" /> Status & Timing
                      </h3>
                      <div className="space-y-3">
                ...
                {activeTab === "RESOURCES" && (
                  <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">
                    ...
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                        <Database className="h-4 w-4" /> Storage Allocation
                      </h3>
                      <div className="space-y-3">
                        <DetailRow
                          label="Total Storage"
                          value={`${vm.storageGb} GB`}
                        />
                ...
                {activeTab === "CONTEXT" && (
                  <div className="grid gap-x-8 gap-y-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                        <Tag className="h-4 w-4" /> AssetOps Context
                      </h3>
                      <div className="space-y-3">
                            ...
                            vm.tags.length > 0 ? (
                              <div className="flex flex-wrap gap-1.5">
                                {vm.tags.map((t) => (
                                  <span
                                    key={t}
                                    className="px-2 py-0.5 bg-muted rounded-md text-[11px] font-medium"
                                  >
                                    {t}
                                  </span>
                                ))}
                    ...
                    <div className="space-y-4">
                      <h3 className="text-xs font-bold uppercase tracking-widest text-muted-foreground flex items-center gap-2">
                        <ShieldCheck className="h-4 w-4" /> Ownership & Usage
                      </h3>
                      <div className="space-y-3">
                        <DetailRow
                          label="Description"
            ...
            <div className="p-4 sm:p-5">
              {vm.components?.length ? (
                <div className="grid gap-3 md:grid-cols-2">
                  {[...vm.components]
                    .sort((a, b) =>
                      a.relationType === "PRIMARY"
                        ? -1
                        : b.relationType === "PRIMARY"
                          ? 1
                          : 0,
                    )
                    .map((component) => (
                      <button
                        ...
                        className={cn(
                          "rounded-xl border border-border/60 bg-background/50 p-3 text-left transition hover:border-primary/40 hover:bg-background",
                          component.relationType === "PRIMARY" &&
                      ...
                      >
                        ...
                        {component.responsibleParty ? (
                          <p className="mt-1 text-[11px] text-muted-foreground">
                            Responsible: {component.responsibleParty}
                          </p>
                        ) : null}
                      </button>
                    ))}
                ...
                <p className="text-sm text-muted-foreground">
                  No Application relationship assigned yet. This VM can remain
                  in inventory as Needs Context until curation is available.
                </p>
              )}
            </div>
          </section>
          ...
          <section className="glass-card overflow-hidden">
            <div className="flex flex-col gap-2 border-b border-border/50 px-4 py-3 sm:px-5">
              <h2 className="flex items-center gap-2 text-base font-bold tracking-tight">
                <ShieldCheck className="h-4 w-4 text-success" /> Guest OS
                Accounts
              </h2>
            </div>
                    ...
                    {vm.guestAccounts.map((account) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-3 py-2.5 sm:px-4">
                            <div className="flex items-center gap-2">
                              ...
                              {canEdit ? (
                                <>
                                  <button
                                    aria-label={`${isRev ? "Hide" : "Reveal"} password for ${account.username}`}
                                    onClick={() => {
                                      void handleRevealPassword(accountId);
                                    }}
                                  ...
                                  <button
                                    ...
                                    onClick={() => {
                                      void handleCopyPassword(account);
                                    }}
                                    className="rounded-md p-1.5 text-muted-foreground hover:bg-muted"
                                  >
                          ...
                          <td className="px-3 py-2.5 sm:px-4">
                            <span className="rounded-md bg-muted px-2.5 py-1 text-[11px] font-semibold">
                              {account.role}
                            </span>
                          </td>
      ...
      <Dialog open={archiveOpen} onOpenChange={setArchiveOpen}>
        <DialogContent className="max-w-md bg-card rounded-2xl">
          ...
          <div className="py-4 text-sm text-muted-foreground">
            ...
            <span className="font-bold text-foreground">{vm.name}</span> and
...
function DetailRow({
  ...
  return (
    <div className="grid grid-cols-[140px_1fr] items-start gap-4 min-w-0">
      <div className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground pt-0.5 shrink-0">
        {label}
      </div>
